...
class HealthMonitor {
  ...
  async checkBlockchain() {
    ...
    try {
      ...
      // Perform multiple connection checks
      const [networkId, blockNumber, chainId, gasPrice] = await Promise.all([
        Number(await this.web3.eth.net.getId()),
        Number(await this.web3.eth.getBlockNumber()),
        Number(await this.web3.eth.getChainId()),
        this.web3.eth.getGasPrice() // gasPrice can remain as BigInt/string
      ]);

      const responseTime = Date.now() - startTime;

      // Validate network configuration
      const configuredNetworkId = config.blockchain.networkId;
      const isNetworkValid = configuredNetworkId
      ...
      const blockchainStatus = {
        ...
        responseTime,
        correctNetwork: isNetworkValid
      };

      // Comprehensive logging
      if (!isNetworkValid) {
        console.warn(`Blockchain Network Mismatch`, {
          expectedNetworkId: configuredNetworkId,
          actualNetworkId: networkId,
          chainId,
          blockNumber
        });

        blockchainStatus.error = `Incorrect network: expected ${configuredNetworkId}, got ${networkId}`;
      } else {
        console.info('Blockchain Connection Healthy', {
          networkId,
          chainId,
          blockNumber,
      ...
      // Update status
      this.status.blockchain = blockchainStatus;
      ...
      if (this.autoRecover) {
        try {
          console.log('Attempting blockchain connection recovery...');
          this.web3 = new Web3(config.blockchain.uri);
          console.log('Blockchain connection reinitialized');
        } catch (recoveryError) {
          console.error('Blockchain connection recovery failed', {
